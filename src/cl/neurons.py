...
class Neurons:
    ...
    def __enter__(self):
        ...
        from ._twin_producer import BiologicalTwinProducer
        ...
        self._sim_mode                    = os.getenv("CL_SDK_SIM_MODE", "replay").lower()
        if self._sim_mode not in {"replay", "surrogate", "biological", "twin"}:
            raise ValueError(
                "CL_SDK_SIM_MODE must be one of: replay, surrogate, biological, twin"
            )
        ...
        if self._sim_mode in {"surrogate", "biological", "twin"}:
            self._data_producer = BiologicalTwinProducer(
    ...
    def _queue_stims(
        ...
        # Producer backends that model biology need an approximate drive
        # strength.  The public StimDesign remains phase-based; internally the
        # twin uses the strongest phase current as the electrode coupling term.
        stim_current_uA         = float(max(abs(current_uA) for current_uA in stim_design._args[1::2]))
