...
def run_grid_task(
    ...
    metadata = run_task_grid(args, TASK_BUILDERS[task](args), output_dir=output_dir, run_id=task)
...
def main(
    ...
    output_root_name = output_root_name or f"snn_reset_final_n50_modal_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}"
    ...
    print(json.dumps({"event": "submitted", "output_root_name": output_root_name, "tasks": selected_tasks}))
    for call in calls:
